...
```{r GHaccess2, eval = FALSE}
# This approach is used in the TRAVIS-CI build environment
# Try to retrieve the token and if successful, prep the access string & checks
# See https://developer.github.com/v3/#authentication
# See https://blogs.infosupport.com/accessing-githubs-rest-api-with-curl/
token_found <- FALSE
token <- Sys.getenv("TRAVIS_CI")
if (token != "") token_found <- TRUE
if (!token_found) message("Could not retrieve token - GET calls will be rate-limited by Github")
if (token_found) {
  gh_access_string <- paste(token, "x-oauth-basic", sep = ":")
  response <- system2("curl", args = c("-u", gh_access_string), stdout = TRUE, stderr = TRUE)
  if (any(grepl("Bad credentials", response))) message("Github access denied")
  if (!any(grepl("Bad credentials", response))) set_config(config(token = token)) # applies to all GET requests
}
```

```{r GHaccess2a, eval = TRUE}
gh_token <- Sys.getenv("TRAVIS_CI")
# set_config(config(token = "$TRAVIS_CI:x-oauth-basic")) # applies to all GET requests
```

```{r GHaccess3, eval = FALSE}
# This approach is used for local builds, not TRAVIS-CI
# Set this token up ahead of time in the R workspace,
# so it is in the environment, available to this Rmd, & not the public!
# Also, see the setupR chunk above where we are careful not to stomp on it
set_config(config(token = github_token)) # applies to all GET requests below
```
...
for (i in 1:ne) {
  if (!is.na(DF$web[i])) {
    webOK[i] <- identical(status_code(GET(paste0(DF$web[i], "?access_token=", gh_token), 200L)))
    webLink[i] <- TRUE
    if (webLink[i] != webOK[i]) badWeb[i] <- TRUE
  }

  if (!is.na(DF$repo[i])) {
    repoOK[i] <- identical(status_code(GET(paste0(DF$repo[i], "?access_token=", gh_token), 200L)))
    repoLink[i] <- TRUE
    if (repoLink[i] != repoOK[i]) badRepo[i] <- TRUE
  }

  if (!is.na(DF$pub[i])) {
    pubOK[i] <- identical(status_code(GET(paste0(DF$pub[i], "?access_token=", gh_token),200L)))
    pubLink[i] <- TRUE
...
LinkReport <- data.frame(name = DF$name, webLink, webOK, repoLink, repoOK, pubLink, pubOK, stringsAsFactors = FALSE)
...
# Write a report
DateReport <- data.frame(name = DF$name, webDate, commitDate, issueDate, updateDate, stringsAsFactors = FALSE)
