...
# The following does not seem to stick; using explict config at each GET
timeout(20) # adjust timeout for GET calls to 20s
```
...
```{r location}
# Figure out the rendering location
# We are either working_locally or we are using GHA
...
if (token_found) {
  ...
  # This is sufficient for working_locally:
...
for (i in 1:ne) {
  if (!is.na(DF$web[i])) {
    webOK[i] <- identical(status_code(GET(DF$web[i], config = list(timeout(20)))), 200L)
    webLink[i] <- TRUE
    if (webLink[i] != webOK[i]) badWeb[i] <- TRUE
  }

  if (!is.na(DF$repo[i])) {
    repoOK[i] <- identical(status_code(GET(DF$repo[i], config = list(timeout(20)))), 200L)
    repoLink[i] <- TRUE
    if (repoLink[i] != repoOK[i]) badRepo[i] <- TRUE
  }

  if (!is.na(DF$pub[i])) {
    pubOK[i] <- identical(status_code(GET(DF$pub[i], config = list(timeout(20)))), 200L)
    pubLink[i] <- TRUE
