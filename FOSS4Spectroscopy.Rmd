...
junk <- virtualenv_create("r-reticulate", python = Sys.which("python3"))
junk <- use_virtualenv("r-reticulate")
junk <- py_available(initialize = TRUE)
# oddly, this next step is not needed & in fact interferes!
# virtualenv_install(envname = "r-reticulate", package = "htmldate")
hdate <- import("htmldate")
...
*The date is found by automatic checking of sites with valid links.  Commits and issues are only checked for Github sites. Web site updates are found using the `Python` package [htmldate](https://pypi.org/project/htmldate/.) Keep in mind the most recent activity may be any branch in a repo, and may be newer than any official releases.*
