...
pubOK <- rep(FALSE, ne)
for (i in 1:ne) {
  if (!is.na(DF$web[i])) webOK[i] <- identical(status_code(GET(DF$web[i])), 200L)
  if (!is.na(DF$repo[i])) repoOK[i] <- identical(status_code(GET(DF$repo[i])), 200L)
  if (!is.na(DF$pub[i])) pubOK[i] <- identical(status_code(GET(DF$pub[i])), 200L)
}
# If URLs are bad they will still be added to the table as hyperlinks, but
# those links will give status 404.
# Need to write a report so maintainers can check & fix
# Also use this info in the status check...
```
