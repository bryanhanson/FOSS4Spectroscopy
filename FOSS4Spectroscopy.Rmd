...
source("Functions/getGHdates.R") # function to get GH most recent commit dates
source("Functions/htmldateR.R") # functions to retrieve most recent date found on generic web pages
source("Functions/goodURL.R") # function to check if a URL can be reached
...
DF <- read_excel("FOSS4Spec.xlsx", na = "NA")
...
# FIRST, check to see if we are on the local/home machine
...
if (local_token_found) { # we are at_home
  token_value <- github_token
  set_config(config(token = token_value))
  at_home <- TRUE
  where <- "home"
}
if (!local_token_found) { # then we must be at_GH
...
# cat("at_GH = ", at_GH, "\n")
# cat("where = ", where, "\n")
# cat("local_token_found = ", local_token_found, "\n")
```

```{r verifyURLs}
# DF <- DF[1:10,] # temporary, for testing
...
for (i in 1:ne) {
  if (!is.na(DF$web[i])) {
    if (at_home) webOK[i] <- goodURL(DF$web[i])
    if (at_GH) {
      GH <- grepl("github\\.com", DF$web[i])
       if (GH) webOK[i] <- goodURL(DF$web[i], config = list( authenticate("bryanhanson", token_value)))
       if (!GH) webOK[i] <- goodURL(DF$web[i])
    }
    webLink[i] <- TRUE
  ...
  if (!is.na(DF$repo[i])) {
    if (at_home) repoOK[i] <- goodURL(DF$repo[i])
    if (at_GH) {
      GH <- grepl("github\\.com", DF$repo[i])
       if (GH) repoOK[i] <- goodURL(DF$repo[i], config = list( authenticate("bryanhanson", token_value)))
       if (!GH) repoOK[i] <- goodURL(DF$repo[i])
    }
    repoLink[i] <- TRUE
  ...
  if (!is.na(DF$pub[i])) {
    if (at_home) pubOK[i] <- goodURL(DF$pub[i])
    if (at_GH) {
      GH <- grepl("github\\.com", DF$pub[i])
       if (GH) pubOK[i] <- goodURL(DF$pub[i], config = list( authenticate("bryanhanson", token_value)))
       if (!GH) pubOK[i] <- goodURL(DF$pub[i])
    }
    pubLink[i] <- TRUE
...
for (i in 1:ne) {
  ...
  if (repoOK[i]) {
    if (repoType[i] == "gh") {
      ...
      tmp <- getGHdates(DF$repo[i], "commits", token_value)
      cnt <- cnt + 1
      if (!is.na(tmp)) commitDate[i] <- ymd(tmp)
      tmp <- getGHdates(DF$repo[i], "issues", token_value)
      cnt <- cnt + 1
