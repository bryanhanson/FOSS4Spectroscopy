...
for (i in 1:ne) {
  if (!is.na(DF$web[i])) {
    webOK[i] <- identical(status_code(
      GET(
        paste0(DF$web[i]), "?access_token=", gh_token),
        200L)
    webLink[i] <- TRUE
  ...
  if (!is.na(DF$repo[i])) {
    repoOK[i] <- identical(status_code(
      GET(
        paste0(DF$repo[i]), "?access_token=", gh_token),
        200L)
    repoLink[i] <- TRUE
  ...
  if (!is.na(DF$pub[i])) {
    pubOK[i] <- identical(status_code(
      GET(
        paste0(DF$pub[i]), "?access_token=", gh_token),
        200L)
    pubLink[i] <- TRUE
