...
for (i in 1:ne) {
  if (!is.na(DF$web[i])) {
    ...
    if (at_TCI) {
      ...
      if (!GH) access_string <- DF$web[i]
      if (GH) access_string <- paste0(DF$web[i], "?access_token=", token_value)
    }
  ...
  if (!is.na(DF$repo[i])) {
    if (at_home) access_string <- DF$repo[i]
    if (at_TCI) {
      GH <- grepl("github\\.com", DF$repo[i])
      if (!GH) access_string <- DF$repo[i]
      if (GH) access_string <- paste0(DF$repo[i], "?access_token=", token_value)
    }
    repoOK[i] <- identical(status_code(GET(access_string)), 200L)
    repoLink[i] <- TRUE
    if (repoLink[i] != repoOK[i]) badRepo[i] <- TRUE
  }

  if (!is.na(DF$pub[i])) {
    if (at_home) access_string <- DF$pub[i]
    if (at_TCI) {
      GH <- grepl("github\\.com", DF$pub[i])
      if (!GH) access_string <- DF$pub[i]
      if (GH) access_string <- paste0(DF$pub[i], "?access_token=", token_value)
    }
    pubOK[i] <- identical(status_code(GET(access_string)), 200L)
    pubLink[i] <- TRUE
...
```{r createTable}
updateDate <- as.POSIXct(rep(NA, ne)) # temporary for testing
...
* *Click on a header to sort the table ("Focus" may be the most useful for browsing)*
* [Abbreviations](#abbreviations) & terms below the table
* The table currently has `r nrow(DF2)` entries; the majority are `R` (`r length(DF2$Lang[DF2$Lang == "R"])`) or `Python` (`r length(DF2$Lang[DF2$Lang == "Py"])`).
...
*The status date is found by automatic checking of sites with valid links.  Commits and issues are only checked for Github sites. Web site updates are found using a simple search for common date formats and may be inaccurate. Keep in mind the most recent activity may be on any branch in a repo, and may be newer than any official releases.*
...
* __Focus:__ The type of spectroscopy the software is focused on.  No entry implies the software is general purpose.  Keep in mind that software designed with a given spectroscopy in mind may still work with other types of spectroscopic data.
...
* __UV-Vis:__ Ultraviolet-visible spectroscopy.
