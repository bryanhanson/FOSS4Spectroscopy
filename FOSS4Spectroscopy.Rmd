...
if (local_token_found) set_config(config(token = github_value))
if (!local_token_found) warning("Did not find a local token")
```
...
for (i in 1:ne) {
  if (!is.na(DF$web[i])) {
    webOK[i] <- goodURL(DF$web[i])
    webLink[i] <- TRUE
    if (webLink[i] != webOK[i]) badWeb[i] <- TRUE
  }

  if (!is.na(DF$repo[i])) {
    repoOK[i] <- goodURL(DF$repo[i])
    repoLink[i] <- TRUE
    if (repoLink[i] != repoOK[i]) badRepo[i] <- TRUE
  }

  if (!is.na(DF$pub[i])) {
    pubOK[i] <- goodURL(DF$pub[i])
    pubLink[i] <- TRUE
